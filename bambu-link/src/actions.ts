    ...
    const CHAMBER_LIGHT_ON = {
        "system": {"sequence_id": "0", "command": "ledctrl", "led_node": "chamber_light", "led_mode": "on",
                "led_on_time": 500, "led_off_time": 500, "loop_times": 0, "interval_time": 0}}
                    ...
                    chamberLightOn: () => {
                        mqqtClient.publish(`device/${serial}/report`, JSON.stringify(CHAMBER_LIGHT_ON));
                    },
                    chamberLightOff: () => {
                        mqqtClient.publish(`device/${serial}/report`, JSON.stringify(CHAMBER_LIGHT_OFF));
                    },
                    getVersion: () => {
                        mqqtClient.publish(`device/${serial}/report`, JSON.stringify(GET_VERSION));
                    }
                    }
                }